...
count=(strlen(ss));
    ...
    printf("Will now print the word from the english dictionay file which must be in the Dir \n");
    printf(" dictionary = words.txt  \n");
    char *corrected_word = correct(ss);
    puts(corrected_word);
    printf(" \n ");

    printf("finished printing the word \n");

/*
    printf("Will now run the same word through the spell function, but hard coded as the word heey");
    printf(" see line 352 where buffer[256]= heey;  is defined in \n");
    char *corrected_word2 = correct(buffer);
    puts(corrected_word2);
    printf(" \n ");
